...
This file exists because on 2026-05-05 19:08 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `323` cumulative in the last 30 days
...
## Top failure modes (all-time, 323 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 698 trips (216%)
2. **timeout** — 129 trips (40%)
3. **api: overloaded/rate-limit** — 22 trips (7%)
4. **permission denied (wrote to server path locally?)** — 21 trips (7%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 58 time(s)
...
- Last update: 2026-05-05 19:08 PDT
- Trips tracked total: 323
