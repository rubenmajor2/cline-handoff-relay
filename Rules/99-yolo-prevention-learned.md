...
This file exists because on 2026-05-27 00:21 PDT a scan of the last Cline task history
showed `3` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `412` cumulative in the last 30 days
(`148` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 632 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1411 trips (223%)
2. **timeout** — 171 trips (27%)
3. **permission denied (wrote to server path locally?)** — 45 trips (7%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (6%)
5. **api: overloaded/rate-limit** — 29 trips (5%)
6. **file/path does not exist** — 23 trips (4%)
7. **tool: generic execution error** — 20 trips (3%)
8. **ssh: connect/timeout** — 16 trips (3%)
9. **mysql query failed** — 15 trips (2%)
10. **sql: unknown column (DESCRIBE target table first)** — 12 trips (2%)
11. **browser_action: failed** — 10 trips (2%)
12. **php: syntax error (run php -l before deploy)** — 8 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 160 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 156 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 81 time(s)
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 44 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 28 time(s)
- `file/path does not exist > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 22 time(s)
- `tool: generic execution error > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 20 time(s)
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 16 time(s)
...
- **file/path does not exist** (23 trips) — see `clinerules_search("file/path does not exist")`
- **tool: generic execution error** (20 trips) — see `clinerules_search("tool")`
- **ssh: connect/timeout** (16 trips) — see `clinerules_search("ssh")`
- **mysql query failed** (15 trips) — see `clinerules_search("mysql query failed")`
- **sql: unknown column (DESCRIBE target table first)** (12 trips) — see `clinerules_search("sql")`
- **browser_action: failed** (10 trips) — see `clinerules_search("browser_action")`
- **php: syntax error (run php -l before deploy)** (8 trips) — see `clinerules_search("php")`
...
- Last update: 2026-05-27 00:21 PDT
- Trips tracked total: 632
