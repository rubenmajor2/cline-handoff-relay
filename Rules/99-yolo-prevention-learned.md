...
This file exists because on 2026-05-26 12:18 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `396` cumulative in the last 30 days
...
## Top failure modes (all-time, 614 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1367 trips (223%)
2. **timeout** — 164 trips (27%)
...
11. **browser_action: failed** — 9 trips (1%)
...
17. **ssh: port 2222 timeout** — 1 trips (0%)
18. **tool: result missing** — 1 trips (0%)

## Most common triple-failure patterns

These are the exact `fail > fail > fail` sequences that have ended tasks:

- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 156 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 146 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 80 time(s)
...
- **browser_action: failed** (9 trips) — see `clinerules_search("browser_action")`
...
- **tool: result missing** (1 trips) — see `clinerules_search("tool")`
...
- Last update: 2026-05-26 12:18 PDT
- Trips tracked total: 614
