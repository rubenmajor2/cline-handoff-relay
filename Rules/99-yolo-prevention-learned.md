...
This file exists because on 2026-06-01 22:37 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `89` cumulative in the last 30 days
...
## Top failure modes (all-time, 89 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 225 trips (253%)
2. **timeout** — 14 trips (16%)
...
8. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 trips (2%)
9. **php: syntax error (run php -l before deploy)** — 1 trips (1%)
10. **api: credit exhausted (escalate, no retry)** — 1 trips (1%)
11. **permission denied (wrote to server path locally?)** — 1 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 12 time(s)
...
- Last update: 2026-06-01 22:37 PDT
- Trips tracked total: 89
