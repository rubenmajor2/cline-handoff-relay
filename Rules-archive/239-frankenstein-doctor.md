...
## THE FEDERATION-FIRST PRINCIPLE — the Federation routes around dead LLMs; the Doctor NEVER hardcodes LLM fixes into the router (Ruben, 2026-07-14)

**The Frankenstein Federation is composed of ALL the LLMs. Its most important job is to route to the next available LLM when one is down.** The Doctor's FIRST priority is to ensure the Federation is doing this correctly. If an LLM is down, that is FINE — the Federation should route around it. The Doctor fixes individual LLMs SECOND, and ONLY on the box itself (vLLM config, Ollama config), NEVER by hardcoding workarounds into the router.

Ruben, verbatim (2026-07-14): *"these windows will try to fix the individual LLM first before resolving the router issue and so what they end up doing is hardcoding LLM fixes which then create problems for the entire router. The process is disjointed. The most important thing is that we make sure that the Federation is able to go to the next available LLM. If an LLM is down, that's fine for the doctor to fix the LLM, but it can't hardcode things into the router in order to solve the immediate issue."*

### The Federation-first protocol (MANDATORY before any individual LLM work)

**Step 0F — Ensure the Federation routes around the dead LLM (BEFORE fixing the LLM itself):**

1. **Probe the Federation's fallback behavior:** with the dead LLM, does the Federation successfully route to the NEXT available LLM? If YES → the Federation is working correctly; the dead LLM is a separate, lower-priority fix. If NO → the Federation has a routing defect that MUST be fixed FIRST.
2. **Fix Federation routing defects ONLY at the Federation/Doorman layer:** lane config, fallback_for_lane, adapter canary health, TOOL_TRACK rungs. These are Federation-level fixes that help ALL traffic, not just one LLM's.
3. **NEVER hardcode a specific model workaround into the router to bypass a dead LLM.** Examples of FORBIDDEN fixes: pinning a route to a specific model name, disabling a model in config.yaml to force fallback, changing tool_rank to demote a dead model (the Federation's health/canary system should handle this automatically). These hardcoded fixes persist after the LLM is fixed and create NEW routing problems.
4. **Individual LLM fixes happen on the BOX, not in the router:** vLLM launch flags, Ollama model pulls, Docker config, systemd units. The router config (config.yaml, registry.yaml, router_hook.py) must NOT change to work around a dead box. The Federation's health system will auto-detect when the box comes back and route to it again.

**The bright-line rule: if your fix touches `/etc/litellm/config.yaml`, `/etc/litellm/frankenstein_registry.yaml`, or `router_hook.py` to work around a dead LLM, you are HARDCODING — STOP.** The only valid router-level fixes are Federation/Doorman behavioral improvements (lane health checks, fallback logic, canary improvements) that help the Federation route around ANY dead LLM, not just this one. File the individual LLM fix as a separate idea for the box itself.
...
0. **Ensure the Federation routes around the dead LLM (Federation-first).** BEFORE any individual LLM fix, verify the Federation is successfully routing traffic to the NEXT available LLM. If the Federation is routing correctly, the patient window should recover on its own — the dead LLM is a separate, lower-priority issue. If the Federation is NOT routing correctly, fix the Federation/Doorman layer (NOT the individual LLM). This is step 0 because it comes before everything else — a working Federation means most "stuck window" issues self-resolve.
...
2026-07-14 — added THE FEDERATION-FIRST PRINCIPLE section + Step 0F in the ladder of intervention. Source: Ruben directive — Doctor windows were fixing individual LLMs first by hardcoding workarounds into the router (config.yaml, registry.yaml, router_hook.py), which created NEW routing problems. The Federation is composed of ALL LLMs and its job is to route to the next available one when one is down. The Doctor's FIRST priority is ensuring the Federation routes correctly around dead LLMs. Individual LLM fixes happen on the BOX (vLLM flags, Ollama config), NEVER as hardcoded router workarounds. Added the bright-line rule: if your fix touches config.yaml/registry.yaml/router_hook.py to work around a dead LLM, you are HARDCODING — STOP. Also confirmed Federation IS already in the Doctor rule (Step 0b, added 2026-07-07) — Ruben asked "if this whole exercise is included within Doctor of Frankenstein" and the answer is YES.
