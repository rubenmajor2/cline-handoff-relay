...
This file exists because on 2026-05-29 12:52 MST a scan of the last Cline task history
showed `14` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `479` cumulative in the last 30 days
(`216` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 729 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1638 trips (225%)
2. **timeout** — 188 trips (26%)
3. **permission denied (wrote to server path locally?)** — 46 trips (6%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (5%)
5. **api: overloaded/rate-limit** — 34 trips (5%)
6. **ssh: connect/timeout** — 30 trips (4%)
7. **file/path does not exist** — 25 trips (3%)
8. **tool: generic execution error** — 25 trips (3%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 189 time(s)
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 177 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 99 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 33 time(s)
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 30 time(s)
- `tool: generic execution error > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 25 time(s)
...
- **ssh: connect/timeout** (30 trips) — see `clinerules_search("ssh")`
- **file/path does not exist** (25 trips) — see `clinerules_search("file/path does not exist")`
- **tool: generic execution error** (25 trips) — see `clinerules_search("tool")`
...
- Last update: 2026-05-29 12:52 MST
- Trips tracked total: 729
- New trips this scan: 14
