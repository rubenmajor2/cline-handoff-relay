...
This file exists because on 2026-05-07 22:51 PDT a scan of the last Cline task history
...
## Top failure modes (all-time, 343 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 742 trips (216%)
2. **timeout** — 105 trips (31%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 52 trips (15%)
4. **api: overloaded/rate-limit** — 23 trips (7%)
5. **permission denied (wrote to server path locally?)** — 20 trips (6%)
6. **file/path does not exist** — 14 trips (4%)
7. **tool: generic execution error** — 12 trips (3%)
...
- Last update: 2026-05-07 22:51 PDT
- Trips tracked total: 343
