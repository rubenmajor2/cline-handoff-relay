...
This file exists because on 2026-06-01 16:36 PDT a scan of the last Cline task history
showed `2` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `69` cumulative in the last 30 days
(`69` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 69 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 182 trips (264%)
2. **timeout** — 9 trips (13%)
3. **api: overloaded/rate-limit** — 2 trips (3%)
4. **replace_in_file: SEARCH did not match file** — 2 trips (3%)
5. **sql: unknown column (DESCRIBE target table first)** — 1 trips (1%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 46 time(s)
...
- `timeout > replace_in_file: SEARCH did not match file > replace_in_file: SEARCH did not match file` — 1 time(s)
...
- Last update: 2026-06-01 16:36 PDT
- Trips tracked total: 69
