...
This file exists because on 2026-05-26 15:19 PDT a scan of the last Cline task history
showed `3` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `402` cumulative in the last 30 days
(`154` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 621 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1384 trips (223%)
2. **timeout** — 167 trips (27%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 159 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 149 time(s)
...
- Last update: 2026-05-26 15:19 PDT
- Trips tracked total: 621
