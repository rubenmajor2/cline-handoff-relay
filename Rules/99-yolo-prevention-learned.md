...
This file exists because on 2026-05-29 09:18 MST a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `438` cumulative in the last 30 days
(`180` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 686 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1536 trips (224%)
...
5. **api: overloaded/rate-limit** — 32 trips (5%)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 31 time(s)
...
- Last update: 2026-05-29 09:18 MST
- Trips tracked total: 686
- New trips this scan: 1
