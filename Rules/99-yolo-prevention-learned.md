...
This file exists because on 2026-05-22 20:53 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `349` cumulative in the last 30 days
(`71` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 519 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1152 trips (222%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 120 time(s)
...
- Last update: 2026-05-22 20:53 PDT
- Trips tracked total: 519
