...
This file exists because on 2026-05-26 01:17 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `390` cumulative in the last 30 days
(`144` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 608 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1356 trips (223%)
2. **timeout** — 158 trips (26%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 151 time(s)
...
- Last update: 2026-05-26 01:17 PDT
- Trips tracked total: 608
