...
- **MCP ssh_command gotchas:** a remote `pkill`/`grep` that matches nothing returns non-zero and the wrapper reports "Command failed" (not fatal). A bare `=` in a remote grep arg and `&` backgrounding both trip the wrapper. Prefer files + setsid. Append `; true` to force a 0 exit when a `pkill`/`pgrep -c` would otherwise return non-zero.
- **pip RESOLVER THRASH on the RunPod image (2026-06-09):** the `runpod/pytorch` image ships torch cu124 + torchvision/torchaudio cu124. Installing torch cu130/cu128 over it makes pip's resolver backtrack ENDLESSLY (16 min, never committed, transformers never installed). FIX: BEFORE installing the pinned stack, `pip uninstall -y torchvision torchaudio` to remove the cu124 pins that anchor the resolver. Then the pinned install commits in ~3 min (unsloth pulls its own torch 2.10.0+cu128, which is fine). Add this uninstall to run_train.sh as the first step. If a pip has already thrashed >5 min with no commit, pkill it, uninstall the conflicts, reinstall clean, then launch the trainer DIRECTLY (deps already in) to skip re-running the install.
- **30-min checkpoints (rule 147):** every run MUST set `save_strategy="steps", save_steps=<~30min worth>, save_total_limit=4` + `resume_from_checkpoint`. Compute save_steps = floor(1800/sec_per_step). A reclaim then restarts from the last ckpt, not zero.
- **Run TWO pods in parallel for a critical run (Ruben 2026-06-09):** B200 spot/secure gets reclaimed often. Keep a slower-but-alive pod (H200) running until the faster pod (B200) shows real progress, THEN kill the slow one. On 2026-06-09 the H200 was reclaimed mid-run while the B200 was still installing — the parallel pod was the survivor.
