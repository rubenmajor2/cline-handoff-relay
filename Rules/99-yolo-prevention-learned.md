...
This file exists because on 2026-06-05 01:50 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `397` cumulative in the last 30 days
(`45` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 730 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1640 trips (225%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 100 time(s)
...
- Last update: 2026-06-05 01:50 PDT
- Trips tracked total: 730
