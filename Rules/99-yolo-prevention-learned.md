...
This file exists because on 2026-05-10 23:08 PDT a scan of the last Cline task history
...
## Top failure modes (all-time, 394 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 870 trips (221%)
...
10. **shell: command not found** — 2 trips (1%)
11. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 2 trips (1%)
12. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 trips (1%)
13. **sql: unknown column (DESCRIBE target table first)** — 1 trips (0%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 88 time(s)
...
- Last update: 2026-05-10 23:08 PDT
- Trips tracked total: 394
