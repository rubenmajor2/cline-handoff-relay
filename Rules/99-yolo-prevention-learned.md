...
This file exists because on 2026-05-29 10:20 MST a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `443` cumulative in the last 30 days
...
## Top failure modes (all-time, 691 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1546 trips (224%)
2. **timeout** — 183 trips (26%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 172 time(s)
...
- Last update: 2026-05-29 10:20 MST
- Trips tracked total: 691
