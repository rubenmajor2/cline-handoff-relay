...
This file exists because on 2026-05-24 00:39 PDT a scan of the last Cline task history
showed `3` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `332` cumulative in the last 30 days
...
## Top failure modes (all-time, 541 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1203 trips (222%)
2. **timeout** — 138 trips (26%)
3. **permission denied (wrote to server path locally?)** — 43 trips (8%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (6%)
5. **api: overloaded/rate-limit** — 28 trips (5%)
6. **file/path does not exist** — 20 trips (4%)
7. **tool: generic execution error** — 20 trips (4%)
8. **mysql query failed** — 14 trips (3%)
9. **ssh: connect/timeout** — 13 trips (2%)
10. **sql: unknown column (DESCRIBE target table first)** — 9 trips (2%)
11. **browser_action: failed** — 6 trips (1%)
12. **shell: command not found** — 4 trips (1%)
13. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
14. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 4 trips (1%)
15. **php: syntax error (run php -l before deploy)** — 3 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 132 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 127 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 73 time(s)
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 42 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 27 time(s)
- `file/path does not exist > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 20 time(s)
...
- `mysql query failed > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 13 time(s)
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 13 time(s)
...
- Last update: 2026-05-24 00:39 PDT
- Trips tracked total: 541
