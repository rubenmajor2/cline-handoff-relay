...
This file exists because on 2026-05-25 11:42 PDT a scan of the last Cline task history
...
(`118` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 577 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1285 trips (223%)
2. **timeout** — 147 trips (25%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 140 time(s)
...
- Last update: 2026-05-25 11:42 PDT
- Trips tracked total: 577
