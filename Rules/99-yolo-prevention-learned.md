...
This file exists because on 2026-05-05 02:20 PDT a scan of the last Cline task history
showed `5` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `307` cumulative in the last 30 days
...
## Top failure modes (all-time, 307 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 658 trips (214%)
2. **timeout** — 125 trips (41%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 50 time(s)
...
- Last update: 2026-05-05 02:20 PDT
- Trips tracked total: 307
