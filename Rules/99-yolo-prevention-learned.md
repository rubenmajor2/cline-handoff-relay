...
This file exists because on 2026-06-01 19:36 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `78` cumulative in the last 30 days
...
## Top failure modes (all-time, 78 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 201 trips (258%)
2. **timeout** — 13 trips (17%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 49 time(s)
...
- Last update: 2026-06-01 19:36 PDT
- Trips tracked total: 78
