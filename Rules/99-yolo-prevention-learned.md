...
This file exists because on 2026-05-24 07:40 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `342` cumulative in the last 30 days
(`99` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 551 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1227 trips (223%)
2. **timeout** — 140 trips (25%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 134 time(s)
...
- Last update: 2026-05-24 07:40 PDT
- Trips tracked total: 551
