...
This file exists because on 2026-06-03 11:39 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `139` cumulative in the last 30 days
(`139` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 139 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 359 trips (258%)
2. **timeout** — 21 trips (15%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 85 time(s)
...
- Last update: 2026-06-03 11:39 PDT
- Trips tracked total: 139
