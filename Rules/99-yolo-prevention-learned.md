...
This file exists because on 2026-05-25 19:15 PDT a scan of the last Cline task history
showed `3` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `382` cumulative in the last 30 days
(`137` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 597 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1330 trips (223%)
2. **timeout** — 156 trips (26%)
...
11. **php: syntax error (run php -l before deploy)** — 8 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 149 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 142 time(s)
...
- Last update: 2026-05-25 19:15 PDT
- Trips tracked total: 597
