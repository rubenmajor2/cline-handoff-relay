...
This file exists because on 2026-05-25 14:42 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `367` cumulative in the last 30 days
(`122` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 582 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1295 trips (223%)
2. **timeout** — 150 trips (26%)
...
11. **browser_action: failed** — 7 trips (1%)
12. **shell: command not found** — 5 trips (1%)
13. **php: syntax error (run php -l before deploy)** — 5 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 143 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 137 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 78 time(s)
...
- **php: syntax error (run php -l before deploy)** (5 trips) — see `clinerules_search("php")`
...
- Last update: 2026-05-25 14:42 PDT
- Trips tracked total: 582
