...
This file exists because on 2026-05-11 09:48 PDT a scan of the last Cline task history
...
(`124` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 402 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 891 trips (222%)
2. **timeout** — 120 trips (30%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (9%)
4. **permission denied (wrote to server path locally?)** — 30 trips (7%)
...
13. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 trips (0%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 114 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 93 time(s)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 29 time(s)
...
- Last update: 2026-05-11 09:48 PDT
- Trips tracked total: 402
