...
This file exists because on 2026-05-19 13:11 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `467` cumulative in the last 30 days
(`57` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 467 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1036 trips (222%)
2. **timeout** — 131 trips (28%)
3. **permission denied (wrote to server path locally?)** — 40 trips (9%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (7%)
5. **api: overloaded/rate-limit** — 26 trips (6%)
6. **file/path does not exist** — 17 trips (4%)
7. **tool: generic execution error** — 15 trips (3%)
8. **mysql query failed** — 13 trips (3%)
9. **ssh: connect/timeout** — 12 trips (3%)
10. **sql: unknown column (DESCRIBE target table first)** — 6 trips (1%)
11. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
12. **php: syntax error (run php -l before deploy)** — 3 trips (1%)
13. **shell: command not found** — 2 trips (0%)
14. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 trips (0%)
15. **safe-deploy: sha drift, re-read file before retry** — 2 trips (0%)
16. **ssh: port 2222 timeout** — 1 trips (0%)
17. **tool: result missing** — 1 trips (0%)
18. **browser_action: failed** — 1 trips (0%)

## Most common triple-failure patterns

These are the exact `fail > fail > fail` sequences that have ended tasks:

- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 125 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 108 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 53 time(s)
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 39 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 25 time(s)
- `file/path does not exist > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 17 time(s)
- `tool: generic execution error > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 15 time(s)
- `mysql query failed > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 12 time(s)
...
- Last update: 2026-05-19 13:11 PDT
- Trips tracked total: 467
