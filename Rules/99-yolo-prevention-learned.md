...
This file exists because on 2026-05-22 01:42 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `506` cumulative in the last 30 days
(`66` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 506 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1122 trips (222%)
...
6. **file/path does not exist** — 19 trips (4%)
...
- `file/path does not exist > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 19 time(s)
...
- Last update: 2026-05-22 01:42 PDT
- Trips tracked total: 506
