...
This file exists because on 2026-05-05 01:19 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `296` cumulative in the last 30 days
...
## Top failure modes (all-time, 296 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 631 trips (213%)
2. **timeout** — 120 trips (41%)
...
6. **ssh: connect/timeout** — 15 trips (5%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 45 time(s)
...
- Last update: 2026-05-05 01:19 PDT
- Trips tracked total: 296
