...
This file exists because on 2026-05-12 18:18 PDT a scan of the last Cline task history
...
## Top failure modes (all-time, 426 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 943 trips (221%)
2. **timeout** — 123 trips (29%)
3. **permission denied (wrote to server path locally?)** — 37 trips (9%)
...
10. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
11. **sql: unknown column (DESCRIBE target table first)** — 3 trips (1%)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 36 time(s)
...
- Last update: 2026-05-12 18:18 PDT
- Trips tracked total: 426
