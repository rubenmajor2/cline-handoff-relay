...
function reindex(db: any, verbose = false): {
  count: number;
  total_bytes: number;
  total_tokens: number;
...
let dbMode: "native" | "fallback" = "native";
let db: any = null;

// In-memory fallback index (used when native better-sqlite3 fails to load)
type InMemRule = RuleRow & { body: string };

let inMemBySlug: Map<string, InMemRule> = new Map();
let inMemByRuleId: Map<string, InMemRule> = new Map();
let inMemAll: InMemRule[] = [];

function buildInMemoryIndex(): void {
  const collected: { full: string; filename: string }[] = [];
  const walk = (dir: string) => {
    if (!fs.existsSync(dir)) return;
    for (const f of fs.readdirSync(dir).sort()) {
...
function woprSyncPost(action: string, body: Record<string, any>): Promise<any> {
...
server.tool(
  "clinerules_push_rule",
  "Push a new or updated rule to the WOPR canonical store (#12285 sync bus). Writes file locally, pushes to WOPR via API, triggers DB upsert + steering cache bust + learner rebuild flag. Use when authoring a rule in Cline that should propagate everywhere.",
  {
    slug: z.string().describe("Rule slug (e.g. '147-my-new-rule'). Used as filename without .md."),
    body: z.string().describe("Full markdown body of the rule."),
    is_hardfloor: z.boolean().default(false).describe("True if this is a hardfloor rule (loads every task)."),
    target_dir: z.enum(["rules", "archive"]).default("archive").describe("'rules'=hardfloor dir, 'archive'=non-hardfloor dir."),
    source_surface: z.string().default("mac_rules").describe("Source surface identifier."),
  },
  async ({ slug, body, is_hardfloor, target_dir, source_surface }) => {
    ...
    try {
      execSync("bash ~/Documents/Cline/scripts/sync_clinerules_to_wopr.sh &", {
...
server.tool(
  "clinerules_sync_status",
  "Show sync status between Mac local index and WOPR canonical store. Reveals drift (rules on Mac but not WOPR, or vice versa). Use to verify #12285 bidirectional sync is healthy.",
  {},
  async () => {
    // Local Mac count
    const localCount = (db.prepare("SELECT COUNT(*) AS n FROM rules").get() as any)?.n ?? 0;
    const localRules = db.prepare("SELECT slug, mtime FROM rules ORDER BY slug").all() as any[];
    ...
    if (woprStatus && !woprStatus.error) {
      ...
      // Drift detection
      const woprSlugs = new Set((woprStatus.recent_events ?? []).map((e: any) => e.slug));
      lines.push(``);
      ...
      if (woprStatus.recent_events?.length) {
        lines.push(`\nRecent events:`);
        for (const e of (woprStatus.recent_events as any[]).slice(0, 5)) {
          lines.push(`  ${e.created_at} ${e.action} ${e.slug} (${e.source_surface})`);
        }
      }

      if (woprStatus.proposed?.length) {
        lines.push(`\nProposed rules pending review: ${woprStatus.proposed.map((p: any) => `${p.status}:${p.n}`).join(", ")}`);
      }
    } else {
...
server.tool(
  "clinerules_propose_rule",
  "Submit a proposed new clinerule to the WOPR pending-review queue (#12285 rule→behavior loop). Use when a pattern in yolo_trips or good-completions suggests a new rule is needed. Ruben reviews proposals at /routes/clinerules_sync_api.php?action=status.",
  {
    slug: z.string().describe("Proposed slug (e.g. 'proposed-147-foo-bar'). Will be prefixed with 'proposed-' if not already."),
    title: z.string().describe("Short title for the proposed rule."),
    body: z.string().describe("Full markdown body of the proposed rule."),
    proposed_by: z.enum(["learner_yolo_pattern", "learner_good_completion", "frankenstein_llm", "manual", "mcp_push"]).default("mcp_push"),
    source_evidence: z.string().default("").describe("Evidence for why this rule is needed (e.g. 'n=15 yolo_trips matching pattern X')."),
  },
