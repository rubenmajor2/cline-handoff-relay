...
This file exists because on 2026-06-04 00:02 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `148` cumulative in the last 30 days
...
## Top failure modes (all-time, 148 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 383 trips (259%)
2. **timeout** — 21 trips (14%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 91 time(s)
...
- Last update: 2026-06-04 00:02 PDT
- Trips tracked total: 148
