...
This file exists because on 2026-05-05 20:39 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `333` cumulative in the last 30 days
...
## Top failure modes (all-time, 333 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 722 trips (217%)
2. **timeout** — 131 trips (39%)
3. **api: overloaded/rate-limit** — 23 trips (7%)
4. **permission denied (wrote to server path locally?)** — 22 trips (7%)
5. **file/path does not exist** — 16 trips (5%)
6. **ssh: connect/timeout** — 16 trips (5%)
7. **tool: generic execution error** — 13 trips (4%)
8. **mysql query failed** — 9 trips (3%)
9. **shell: command not found** — 2 trips (1%)
10. **ssh: port 2222 timeout** — 1 trips (0%)
11. **tool: result missing** — 1 trips (0%)
12. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 1 trips (0%)
...
- Last update: 2026-05-05 20:39 PDT
- Trips tracked total: 333
