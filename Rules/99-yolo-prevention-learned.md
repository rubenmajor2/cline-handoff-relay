...
This file exists because on 2026-05-23 19:39 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `328` cumulative in the last 30 days
...
## Top failure modes (all-time, 534 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1186 trips (222%)
...
10. **sql: unknown column (DESCRIBE target table first)** — 8 trips (1%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 124 time(s)
...
- Last update: 2026-05-23 19:39 PDT
- Trips tracked total: 534
