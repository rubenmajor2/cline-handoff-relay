...
This file exists because on 2026-07-27 21:39 PDT a scan of the last Cline task history
...
(`6` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 201 trips)
...
1. **no-tool-use: model typed prose instead of calling a tool** — 524 occurrences (261% of 201 trips)
2. **timeout** — 33 occurrences (16% of 201 trips)
3. **ssh: connect/timeout** — 5 occurrences (2% of 201 trips)
4. **replace_in_file: SEARCH did not match file** — 4 occurrences (2% of 201 trips)
5. **permission denied (wrote to server path locally?)** — 4 occurrences (2% of 201 trips)
6. **api: overloaded/rate-limit** — 2 occurrences (1% of 201 trips)
7. **sql: unknown column (DESCRIBE target table first)** — 2 occurrences (1% of 201 trips)
8. **tool: generic execution error** — 2 occurrences (1% of 201 trips)
9. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 occurrences (1% of 201 trips)
10. **php: syntax error (run php -l before deploy)** — 1 occurrences (0% of 201 trips)
11. **api: credit exhausted (escalate, no retry)** — 1 occurrences (0% of 201 trips)
12. **file/path does not exist** — 1 occurrences (0% of 201 trips)

## Most common triple-failure patterns

These are the exact `fail > fail > fail` sequences that have ended tasks:

- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 138 time(s)
...
- `moonshotai/kimi-k3` — 2 trip(s)
- `frankenstein-llm` — 1 trip(s)
- `claude-fable-5:1m` — 1 trip(s)
- `claude-opus-5:1m` — 1 trip(s)
...
- Last update: 2026-07-27 21:39 PDT
- Trips tracked total: 201
