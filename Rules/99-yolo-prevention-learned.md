...
This file exists because on 2026-05-13 00:50 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `431` cumulative in the last 30 days
...
## Top failure modes (all-time, 431 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 955 trips (222%)
2. **timeout** — 123 trips (29%)
3. **permission denied (wrote to server path locally?)** — 38 trips (9%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 46 time(s)
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 37 time(s)
...
- Last update: 2026-05-13 00:50 PDT
- Trips tracked total: 431
