...
10. **Rule 317 Reversal Log section.** Every completion MUST contain a `# Reversal Log` section (idea #25888, Ruben approved 2026-08-12). If the window had no within-window conclusion flips, write `No reversals this window.`. If a flip occurred (MUTEX test: initial and corrected conclusions can't both be true), list each as `- initial → corrected | RCA bucket (wrong premise|stale assumption|unread source|insufficient probe|scope error) | causal rule updated (file/slug) or filed idea #NNNN`. If the validator returns `R317_REVERSAL_LOG`, the completion is BLOCKED. The section is written ONCE at completion time — zero per-turn cost.
