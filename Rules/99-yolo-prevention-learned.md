...
This file exists because on 2026-06-02 14:34 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `134` cumulative in the last 30 days
(`134` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 134 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 344 trips (257%)
2. **timeout** — 21 trips (16%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 80 time(s)
...
- Last update: 2026-06-02 14:34 PDT
- Trips tracked total: 134
