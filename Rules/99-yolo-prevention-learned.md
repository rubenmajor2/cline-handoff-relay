...
This file exists because on 2026-05-22 22:56 PDT a scan of the last Cline task history
...
## Top failure modes (all-time, 526 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1168 trips (222%)
2. **timeout** — 137 trips (26%)
...
10. **sql: unknown column (DESCRIBE target table first)** — 7 trips (1%)
11. **shell: command not found** — 4 trips (1%)
12. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
13. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 4 trips (1%)
14. **browser_action: failed** — 4 trips (1%)
15. **php: syntax error (run php -l before deploy)** — 3 trips (1%)
16. **safe-deploy: sha drift, re-read file before retry** — 2 trips (0%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 122 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 70 time(s)
...
- Last update: 2026-05-22 22:56 PDT
- Trips tracked total: 526
