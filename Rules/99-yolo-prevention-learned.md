...
This file exists because on 2026-05-10 20:08 PDT a scan of the last Cline task history
...
(`114` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 389 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 858 trips (221%)
2. **timeout** — 118 trips (30%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (9%)
4. **permission denied (wrote to server path locally?)** — 29 trips (7%)
5. **api: overloaded/rate-limit** — 24 trips (6%)
6. **file/path does not exist** — 15 trips (4%)
7. **tool: generic execution error** — 14 trips (4%)
8. **ssh: connect/timeout** — 12 trips (3%)
9. **mysql query failed** — 9 trips (2%)
10. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 2 trips (1%)
11. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 trips (1%)
12. **sql: unknown column (DESCRIBE target table first)** — 1 trips (0%)
13. **shell: command not found** — 1 trips (0%)
14. **ssh: port 2222 timeout** — 1 trips (0%)
15. **tool: result missing** — 1 trips (0%)
16. **php: syntax error (run php -l before deploy)** — 1 trips (0%)

## Most common triple-failure patterns

These are the exact `fail > fail > fail` sequences that have ended tasks:

- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 112 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 86 time(s)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 28 time(s)
...
- Last update: 2026-05-10 20:08 PDT
- Trips tracked total: 389
