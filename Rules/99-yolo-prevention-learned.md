...
This file exists because on 2026-06-02 01:37 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `123` cumulative in the last 30 days
...
## Top failure modes (all-time, 123 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 311 trips (253%)
2. **timeout** — 21 trips (17%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 69 time(s)
...
- Last update: 2026-06-02 01:37 PDT
- Trips tracked total: 123
