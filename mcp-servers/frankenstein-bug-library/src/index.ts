...
interface SessionGateEntry {
  symptom: string;
  result: "KNOWN_REPAIR" | "NOVEL_SYMPTOM" | "KNOWN_PATTERN";
  calledAt: Date;
  incidentId?: string;
  sessionToken: string;
}

const sessionGateLog: SessionGateEntry[] = [];
...
function gateLastEntry(): SessionGateEntry | null {
  const cutoff = new Date(Date.now() - SESSION_GATE_TTL_MS);
...
function esc(s: string, maxLen = 1000): string {
...
function woprQuery(sql: string): Record<string, string>[] {
  ...
  let raw: string;
  try {
    raw = execSync(cmd, { timeout: 15_000, encoding: "utf8", shell: "/bin/bash" });
  } catch (e: any) {
 ...
 */
function woprExecB64(sql: string): string {
 ...
 */
function woprShell(cmd: string): string {
  ...
  const sshCmd = `printf '%s' ${JSON.stringify(b64)} | ssh -p 2222 \
    ...
    "base64 -d | bash"`;
  try {
    return execSync(sshCmd, { timeout: 30_000, encoding: "utf8", shell: "/bin/bash" });
  } catch (e: any) {
 ...
 */
function buildLikeWhere(keywords: string[], columns: string[]): string {
...
function findDuplicateIncident(
  symptom: string,
  problemKey: string
): DuplicateMatch | null {
  ...
  if (problemKey.trim()) {
    const rows = queryRows(
      `SELECT id, problem_key, seen_count, LEFT(resolution,400) AS resolution, status,
    ...
    if (rows.length) return rows[0] as unknown as DuplicateMatch;
  }
 ...
 */
function incrementSeenCount(incidentId: string): boolean {
  try {
    woprExecB64(
      `UPDATE frankenstein_router_incidents
...
// ─── RULE-147 GATE EVALUATOR (FORCING FUNCTION #4) ──────────────────────────

interface Rule147Evaluation {
  gatePath: "48h_freshness" | "three_gs" | "blocked";
  canAutoApply: boolean;
  reasons: string[];
  verdict: string;
}

function evaluateRule147(incident: Record<string, string>): Rule147Evaluation {
  ...
  const failed = [];
  if (!g1_confidence) failed.push("G1(resolved status missing)");
  if (!g2_reversible) failed.push("G2(no recorded resolution)");
  if (!g3_single_surface) failed.push("G3(multi-surface impact)");
...
function writeGateSessionToDB(entry: SessionGateEntry): void {
...
const server = new McpServer({ name: "frankenstein-bug-library", version: VERSION });

// ─── Tool 1: bug_library_search (unchanged from v0.1.0) ──────────────────────

server.tool(
  "bug_library_search",
  `Search frankenstein_router_incidents for past incidents matching a symptom keyword.
...
(Per clinerule 156 + idea #12619.)`,
  {
    symptom: z.string().describe(
      "Symptom to search for. Examples: 'empty unparsable', 'garbage 200', 'restart storm', 'dead rung 404', 'timeout stall'."
    ),
  ...
  async ({ symptom, limit, status_filter }) => {
    ...
    const lines: string[] = [`🔎 bug_library_search("${symptom}") — ${rows.length} match(es):`, ""];
    ...
    for (const r of rows) {
      ...
      if (r.last_seen_at) lines.push(`   last_seen: ${r.last_seen_at}`);
      lines.push("");
    }
...
server.tool(
  "bug_library_check_before_fix",
  `MANDATORY first call before diagnosing ANY frankenstein-llm / LLM-routing symptom.
...
first, routing-fix tools return GATE_BLOCKED.`,
  {
    symptom: z.string().describe(
      "The observed symptom in 1-3 sentences. Examples: 'Invalid API Response empty/unparsable', 'garbage 200 spill burst on executor chain', '404 dead rung on frankenstein-llm fallback'."
    ),
  },
  async ({ symptom }) => {
    const keywords = symptom
    ...
    let resultType: SessionGateEntry["result"];
...
server.tool(
  "bug_library_record",
  `Record a new frankenstein-llm / LLM-routing incident (or update an existing one).
...
record's resolution instead of creating a duplicate.`,
  {
    symptom: z.string().describe("1-3 sentence description of the observed symptom."),
    diagnosis: z.string().default("").describe("Root cause identified (if known)."),
    resolution: z.string().default("").describe("Steps taken or recommended to resolve. Be specific — this is what future agents will apply."),
    evidence: z.string().default("").describe("Supporting evidence: log lines, curl output, audit rows, etc."),
    problem_key: z.string().default("").describe("Short slug key (e.g. 'garbage_200_spill_2026_06_15'). Auto-generated from symptom if blank."),
    status: z.enum(["open", "investigating", "resolved"]).default("open").describe("Resolution status."),
    created_by: z.string().default("cline").describe("Who is recording this (default: cline)."),
    force_insert: z.boolean().default(false).describe("Set true to bypass dedup and force a new row (use only when intentionally recording a distinct incident with the same key)."),
  },
  async ({ symptom, diagnosis, resolution, evidence, problem_key, status, created_by, force_insert }) => {
    ...
    try {
      ...
      if (writeOk) {
        const verify = woprExecB64(
          `SELECT id FROM frankenstein_router_incidents WHERE id=${parseInt(newId, 10)} LIMIT 1;`
        ).trim();
...
server.tool(
  "bug_library_stats",
  "Quick stats on the frankenstein bug library: total incidents, breakdown by status, most recent entry. Use to verify the library is healthy.",
  {},
  async () => {
    const sql = `
      SELECT
...
server.tool(
  "bug_library_gate_status",
  `Query the session gate state (idea #13082 FORCING-FUNCTION GATE).
...
if called while GATE_BLOCKED, forcing the agent to call check_before_fix first.`,
  {},
  async () => {
...
server.tool(
  "bug_library_repeat_rate",
  `Repeat-rate KPI: shows which problems recur most, by seen_count.
...
(Idea #13082 structural KPI #5)`,
  {
    limit: z.number().int().min(1).max(50).default(20).describe("Max rows (default 20)."),
    min_seen: z.number().int().min(1).default(1).describe("Only show incidents seen at least N times (default 1 = all)."),
  },
...
server.tool(
  "bug_library_tier_gate",
  `Evaluates rule-147 safety gates on a bug-library incident STRUCTURALLY (not advisory).
...
(Idea #13082 structural gate #4, implements clinerule 147)`,
  {
    incident_id: z.number().int().describe("frankenstein_router_incidents.id to evaluate."),
  },
  async ({ incident_id }) => {
    const rows = queryRows(
      `SELECT
...
server.tool(
  "bug_library_kaison_replay",
  `KAISON AUTO-APPLY: replay the recorded verbatim resolution for a KNOWN_REPAIR incident.
...
(Idea #13082 structural gate #3, implements clinerule 147)`,
  {
    incident_id: z.number().int().describe("frankenstein_router_incidents.id to replay."),
    dry_run: z.boolean().default(true).describe("true (default) = preview only. false = execute the repair."),
    pre_state_note: z.string().default("").describe("Brief description of current system state before repair (for reversal snapshot)."),
  },
  async ({ incident_id, dry_run, pre_state_note }) => {
    ...
    const runCountRows = queryRows(
      `SELECT COUNT(*) AS cnt FROM kaison_reversal_snapshots
    ...
    try {
      ...
      if (/^\d+$/.test(snapLine)) snapshotId = snapLine;
    } catch (e: any) {
      return {
        content: [{
          type: "text",
    ...
    for (const cmd of resolutionLines) {
      ...
      try {
        ...
        if (out.trim()) results.push(`    → ${out.trim().slice(0, 120)}`);
      } catch (e: any) {
    ...
    try {
      woprExecB64(
        `UPDATE kaison_reversal_snapshots SET outcome='${outcome}' WHERE id=${parseInt(snapshotId, 10)};`
      );
    } catch { /* non-fatal */ }
    ...
    try {
      woprExecB64(
        `INSERT INTO kaison_watcher_runs (logs_scanned, signatures_detected, incidents_matched, actions_taken, dry_run, summary)
...
(async () => {
  ...
  console.error(
    `[bug-library] v${VERSION} stdio connected · frankenstein_router_incidents on WOPR · ` +
