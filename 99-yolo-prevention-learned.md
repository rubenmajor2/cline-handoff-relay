...
This file exists because on 2026-05-02 17:04 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `268` cumulative in the last 30 days
...
## Top failure modes (all-time, 268 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 570 trips (213%)
2. **timeout** — 102 trips (38%)
3. **api: overloaded/rate-limit** — 21 trips (8%)
4. **permission denied (wrote to server path locally?)** — 19 trips (7%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 96 time(s)
...
- Last update: 2026-05-02 17:04 PDT
- Trips tracked total: 268
