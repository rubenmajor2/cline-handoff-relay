...
This file exists because on 2026-05-13 18:35 PDT a scan of the last Cline task history
...
(`101` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 437 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 967 trips (221%)
2. **timeout** — 125 trips (29%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 119 time(s)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 47 time(s)
...
- Last update: 2026-05-13 18:35 PDT
- Trips tracked total: 437
