...
function loadDiskCache() {
    try {
        ...
        for (const a of CACHED_ACTIONS) {
            if (parsed[a] && typeof parsed[a].fetched_at === "number")
                cache[a] = parsed[a];
        }
...
function persistDiskCache() {
    try {
        ...
        console.error(`[fleet-state-mcp] persistDiskCache failed (non-fatal): ${e.message}`);
    }
}
// Raw remote fetch — used ONLY by the background refresher and by fleet_act.
// Never called on a read tool's hot path.
async function remoteCall(action, params = {}, timeoutMs = REFRESH_TIMEOUT_MS) {
...
function readCached(action) {
    ...
    if (!entry || entry.fetched_at === 0) {
        return {
            error: "cache_cold",
            action,
            note: "Snapshot not yet populated (remote refresh has not succeeded since this process started). " +
    ...
    const meta = {
        _cache: {
            ...
            note: "Served from the local fleet snapshot (refreshed every ~" +
...
const TOOLS = [
    ...
    {
        name: "fleet_now",
        description: "Live aggregate snapshot of EMSU fleet: host heartbeat ages, llm_call_log spend by model (last 24h), llm spend by surface (last 1h), recent fleet/runpod events, active RunPod pods, recent Fleet Agent decisions. Served from the local snapshot (refreshed ~60s) with an _cache.age_seconds staleness marker. Use to answer 'what is the fleet doing right now'.",
        inputSchema: { type: "object", properties: {}, required: [] },
    },
    {
    ...
    },
    {
        ...
        description: "Queryable EMSU LLM routing map — idea #10160. Returns per-surface call stats (DEDUPED by request_id — eliminates 71-190x raw_rows inflation from streaming chunks), transport type, forced_claude flag, local_eligible flag, and corrected session facts. Served from the local snapshot. " +
...
function makeServer() {
    const server = new Server({ name: "fleet-state-mcp", version: "0.4.0" }, { capabilities: { tools: {} } });
...
loadDiskCache();
refreshAll().catch((e) => console.error(`[fleet-state-mcp] initial refresh failed: ${e.message}`));
const refreshTimer = setInterval(() => {
    refreshAll().catch((e) => console.error(`[fleet-state-mcp] refresh failed: ${e.message}`));
}, REFRESH_MS);
refreshTimer.unref?.(); // don't keep the event loop alive on this alone
app.listen(PORT, () => {
    console.error(`[fleet-state-mcp] v0.4.0 native streamableHttp on :${PORT}/mcp ` +
