...
<<<<<<< Updated upstream
This file exists because on 2026-05-03 23:00 MST a scan of the last Cline task history
=======
This file exists because on 2026-05-03 22:34 PDT a scan of the last Cline task history
>>>>>>> Stashed changes
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `276` cumulative in the last 30 days
...
## Top failure modes (all-time, 276 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 586 trips (212%)
2. **timeout** — 108 trips (39%)
3. **api: overloaded/rate-limit** — 21 trips (8%)
4. **permission denied (wrote to server path locally?)** — 20 trips (7%)
5. **file/path does not exist** — 15 trips (5%)
6. **ssh: connect/timeout** — 14 trips (5%)
7. **tool: generic execution error** — 12 trips (4%)
8. **mysql query failed** — 7 trips (3%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 40 time(s)
...
<<<<<<< Updated upstream
- Last update: 2026-05-03 23:00 MST
=======
- Last update: 2026-05-03 22:34 PDT
>>>>>>> Stashed changes
- Trips tracked total: 276
