...
This file exists because on 2026-05-26 04:17 PDT a scan of the last Cline task history
...
(`146` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 610 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1359 trips (223%)
2. **timeout** — 160 trips (26%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 152 time(s)
...
- Last update: 2026-05-26 04:17 PDT
- Trips tracked total: 610
