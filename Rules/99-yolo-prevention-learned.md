...
This file exists because on 2026-05-17 11:33 PDT a scan of the last Cline task history
showed `2` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `456` cumulative in the last 30 days
(`70` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 456 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1010 trips (221%)
2. **timeout** — 128 trips (28%)
3. **permission denied (wrote to server path locally?)** — 40 trips (9%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (8%)
5. **api: overloaded/rate-limit** — 25 trips (5%)
...
10. **sql: unknown column (DESCRIBE target table first)** — 4 trips (1%)
11. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
12. **shell: command not found** — 2 trips (0%)
13. **php: syntax error (run php -l before deploy)** — 2 trips (0%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 122 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 104 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 24 time(s)
...
- Last update: 2026-05-17 11:33 PDT
- Trips tracked total: 456
