...
This file exists because on 2026-05-12 16:18 PDT a scan of the last Cline task history
...
## Top failure modes (all-time, 419 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 928 trips (221%)
2. **timeout** — 123 trips (29%)
3. **permission denied (wrote to server path locally?)** — 35 trips (8%)
...
7. **tool: generic execution error** — 14 trips (3%)
8. **ssh: connect/timeout** — 12 trips (3%)
9. **mysql query failed** — 9 trips (2%)
10. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 3 trips (1%)
11. **sql: unknown column (DESCRIBE target table first)** — 2 trips (0%)
...
15. **ssh: port 2222 timeout** — 1 trips (0%)
16. **tool: result missing** — 1 trips (0%)
17. **safe-deploy: sha drift, re-read file before retry** — 1 trips (0%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 96 time(s)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 34 time(s)
...
- `tool: generic execution error > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 14 time(s)
...
- Last update: 2026-05-12 16:18 PDT
- Trips tracked total: 419
