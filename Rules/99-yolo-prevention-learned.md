...
This file exists because on 2026-06-01 23:37 PDT a scan of the last Cline task history
showed `3` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `93` cumulative in the last 30 days
...
## Top failure modes (all-time, 93 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 233 trips (251%)
2. **timeout** — 17 trips (18%)
3. **ssh: connect/timeout** — 4 trips (4%)
4. **replace_in_file: SEARCH did not match file** — 4 trips (4%)
5. **api: overloaded/rate-limit** — 2 trips (2%)
6. **sql: unknown column (DESCRIBE target table first)** — 2 trips (2%)
7. **permission denied (wrote to server path locally?)** — 2 trips (2%)
8. **tool: generic execution error** — 2 trips (2%)
9. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 trips (2%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 51 time(s)
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 15 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 9 time(s)
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 3 time(s)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 2 time(s)
...
- **permission denied (wrote to server path locally?)** (2 trips) — see `clinerules_search("permission denied (wrote to server path locally?)")`
...
- Last update: 2026-06-01 23:37 PDT
- Trips tracked total: 93
