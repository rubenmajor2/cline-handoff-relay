...
This file exists because on 2026-05-28 18:57 MST a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `422` cumulative in the last 30 days
(`167` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 667 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1492 trips (224%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 167 time(s)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 89 time(s)
...
- Last update: 2026-05-28 18:57 MST
- Trips tracked total: 667
