...
This file exists because on 2026-06-01 18:36 PDT a scan of the last Cline task history
showed `5` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `75` cumulative in the last 30 days
...
## Top failure modes (all-time, 75 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 194 trips (259%)
2. **timeout** — 11 trips (15%)
3. **replace_in_file: SEARCH did not match file** — 4 trips (5%)
4. **api: overloaded/rate-limit** — 2 trips (3%)
5. **sql: unknown column (DESCRIBE target table first)** — 2 trips (3%)
6. **ssh: connect/timeout** — 1 trips (1%)
7. **php: syntax error (run php -l before deploy)** — 1 trips (1%)
8. **api: credit exhausted (escalate, no retry)** — 1 trips (1%)
9. **permission denied (wrote to server path locally?)** — 1 trips (1%)

## Most common triple-failure patterns

These are the exact `fail > fail > fail` sequences that have ended tasks:

- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 48 time(s)
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 9 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 8 time(s)
...
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 1 time(s)
...
- **ssh: connect/timeout** (1 trips) — see `clinerules_search("ssh")`
...
- **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — see `clinerules_search("safe-deploy")`
...
- Last update: 2026-06-01 18:36 PDT
- Trips tracked total: 75
