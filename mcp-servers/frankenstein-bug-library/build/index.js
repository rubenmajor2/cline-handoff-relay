...
const VALID_SEVERITIES = ["P0_critical", "P1_high", "P2_medium", "P3_low"];
server.tool("bug_library_browse", `Faceted browser for frankenstein_router_incidents — filters by domain, category, severity, and status.
...
(Added 2026-07-31 as part of organization overhaul — replaces ad-hoc "grep the problem_key" pattern.)`, {
    ...
    status_filter: zod_1.z
        ...
        .default("all")
        .describe("Filter by resolution status. Default: all."),
    limit: zod_1.z.number().int().min(1).max(50).default(15).describe("Max results (default 15)."),
}, async ({ domain, severity, status_filter, limit }) => {
    const clauses = ["1=1"];
    if (domain)
        clauses.push(`domain = '${esc(domain, 40)}'`);
    if (severity)
        clauses.push(`severity = '${esc(severity, 15)}'`);
    if (status_filter !== "all")
        clauses.push(`status = '${status_filter}'`);
    ...
    for (const r of rows) {
        ...
        if (r.resolution)
            lines.push(`   fix: ${r.resolution}`);
        if (r.last_seen_at)
            lines.push(`   last: ${r.last_seen_at}`);
        lines.push("");
    }
