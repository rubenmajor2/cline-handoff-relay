...
This file exists because on 2026-05-19 22:51 PDT a scan of the last Cline task history
showed `7` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `480` cumulative in the last 30 days
...
## Top failure modes (all-time, 480 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1064 trips (222%)
2. **timeout** — 132 trips (28%)
3. **permission denied (wrote to server path locally?)** — 40 trips (8%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (7%)
5. **api: overloaded/rate-limit** — 27 trips (6%)
6. **file/path does not exist** — 17 trips (4%)
7. **tool: generic execution error** — 15 trips (3%)
8. **mysql query failed** — 13 trips (3%)
9. **ssh: connect/timeout** — 12 trips (2%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 126 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 110 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 61 time(s)
...
- Last update: 2026-05-19 22:51 PDT
- Trips tracked total: 480
- New trips this scan: 7
