...
(Per clinerule 156 + idea #12619.)`,
  ...
  async ({ symptom, limit, status_filter }) => {
    ...
    lines.push(
      "If a resolved row matches your case, apply its resolution. If novel, record with bug_library_record()."
    );
...
Also call when a previously-investigating incident is now resolved.`,
  {
    ...
    problem_key: z
      ...
      .default("")
      .describe(
        "Short slug key (e.g. 'garbage_200_spill_2026_06_15'). Auto-generated from symptom if blank."
      ),
    status: z
  ...
  async ({ symptom, diagnosis, resolution, evidence, problem_key, status, created_by }) => {
    ...
    const esc = (s: string) => s.replace(/\\/g, "\\\\").replace(/'/g, "\\'").slice(0, 1000);
