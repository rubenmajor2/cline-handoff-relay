...
This file exists because on 2026-06-02 00:37 PDT a scan of the last Cline task history
showed `17` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `120` cumulative in the last 30 days
...
## Top failure modes (all-time, 120 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 302 trips (252%)
2. **timeout** — 21 trips (18%)
3. **ssh: connect/timeout** — 5 trips (4%)
4. **replace_in_file: SEARCH did not match file** — 4 trips (3%)
5. **permission denied (wrote to server path locally?)** — 4 trips (3%)
6. **api: overloaded/rate-limit** — 2 trips (2%)
7. **sql: unknown column (DESCRIBE target table first)** — 2 trips (2%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 66 time(s)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 14 time(s)
...
- Last update: 2026-06-02 00:37 PDT
- Trips tracked total: 120
- New trips this scan: 17
