...
function amendRuleOnDisk(slugOrId, taskId, rcaBucket, note, triggerPattern) {
    ...
    try {
        const recent = db.prepare(`SELECT rca_bucket, reversal_note FROM rule_amend WHERE slug = ? ORDER BY id DESC LIMIT 12`).all(row.slug);
...
server.tool("clinerules_log_probe", "SESSION-PROBE LEDGER WRITE (idea #29011). Log a real tool probe you ran THIS session (tool name + a short artifact snippet of what it returned). The validator's freshness gate cross-checks each '(verified: ...)' marker in your completion against probes logged under the same task_id — markers with no matching probe fail as R29011_STALE_EVIDENCE. Log probes as you work; a task with zero logged probes skips the freshness gate entirely (form-only checking still applies).", {
    task_id: zod_1.z.string().describe("The Cline task id this probe belongs to."),
    tool: zod_1.z.string().describe("Tool you ran (e.g. ssh_command, execute_query, npm run build, curl, reconcile_ideas)."),
    artifact: zod_1.z.string().optional().describe("Short snippet of what the tool returned (a number, HTTP code, row, hash, filename)."),
}, async ({ task_id, tool, artifact }) => {
    const row = db.prepare(`INSERT INTO session_probes (task_id, tool, artifact) VALUES (?,?,?)`).run(task_id, tool.slice(0, 200), (artifact || "").slice(0, 1000));
    const n = db.prepare(`SELECT COUNT(*) AS n FROM session_probes WHERE task_id = ?`).get(task_id);
    return {
        content: [{
                type: "text",
...
server.tool("clinerules_false_claim_meter", "UNIFIED FALSE-CLAIM METER (idea #29012). One rollup over ALL false-claim streams with per-source labels: human-corrections (Ruben said it was false), gate-blocked (caught before ship, local gate_blocks), machine-detected (WOPR executor/kaizen corrections). The success metric is the ratio: gate blocks rising while human corrections fall means lies are being caught before they reach Ruben.", {
    days: zod_1.z.number().int().min(1).max(90).default(1).describe("Rollup window in days (default 1 = today-ish)."),
}, async ({ days }) => {
    const lines = [`UNIFIED FALSE-CLAIM METER (idea #29012) — trailing ${days}d`];
    try {
        const human = db.prepare(`SELECT COUNT(*) AS n FROM human_corrections WHERE created_at >= datetime('now', ?)`).get(`-${days} day`);
        const gate = db.prepare(`SELECT COUNT(*) AS n FROM gate_blocks WHERE blocked_at >= datetime('now', ?)`).get(`-${days} day`);
    ...
    try {
        ...
        const resp = await fetch(`https://www.emsuniversity.com/emtskills/api/false_claim_rollup.php?days=${days}`, { signal: ctrl.signal });
        clearTimeout(t);
        ...
        if (data && data.ok) {
            ...
            lines.push(`TOTAL (all streams): ${Number(data.total_machine ?? 0) + Number(db.prepare(`SELECT COUNT(*) AS n FROM human_corrections WHERE created_at >= datetime('now', ?)`).get(`-${days} day`).n) + Number(db.prepare(`SELECT COUNT(*) AS n FROM gate_blocks WHERE blocked_at >= datetime('now', ?)`).get(`-${days} day`).n)}`);
        }
        else {
...
server.tool("clinerules_validate_completion", "PRE-COMPLETION GATE (idea #16224): Validates that a pending attempt_completion result complies with rule 91 (PICKUP PROMPT block required). Call BEFORE attempt_completion. Returns pass/fail with specific violations — if pass=false, fix the listed failures before shipping.", {
    ...
    if (task_id && task_id.trim().length > 0) {
        try {
            ...
            const allProbes = db.prepare(`SELECT tool, artifact, source FROM session_probes WHERE task_id = ? ORDER BY id DESC LIMIT 400`).all(task_id);
            ...
            if (probes.length > 0) {
                ...
                while ((mk = mkRe.exec(result_text)) !== null) {
                    ...
                    const tokens = (inner.match(/[a-z0-9_./:-]{4,}/g) || []).filter((t) => !/^(?:verified|probed|measured|confirmed|returned|status|https?|with|from|this|that|then|call|tool)$/.test(t));
                    const fresh = tokens.some((t) => probeText.includes(t));
                    if (!fresh && tokens.length > 0) {
                        const clipped = mk[0].slice(0, 80);
                        if (!staleMarkers.includes(clipped))
                            staleMarkers.push(clipped);
                    }
                }
                if (staleMarkers.length > 0) {
                    failures.push(`R29011_STALE_EVIDENCE: ${staleMarkers.length} '(verified: ...)' marker(s) do not correspond to ANY probe logged for task ${task_id} via clinerules_log_probe: ` +
                        staleMarkers.map((s) => `"${s}"`).join("; ") + `. ` +
