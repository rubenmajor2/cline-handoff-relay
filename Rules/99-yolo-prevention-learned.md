...
This file exists because on 2026-05-29 11:21 MST a scan of the last Cline task history
showed `5` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `448` cumulative in the last 30 days
(`184` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 697 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1561 trips (224%)
2. **timeout** — 185 trips (27%)
...
5. **api: overloaded/rate-limit** — 33 trips (5%)
6. **file/path does not exist** — 25 trips (4%)
7. **tool: generic execution error** — 25 trips (4%)
8. **ssh: connect/timeout** — 20 trips (3%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 176 time(s)
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 174 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 94 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 32 time(s)
...
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 20 time(s)
...
- **ssh: connect/timeout** (20 trips) — see `clinerules_search("ssh")`
...
- Last update: 2026-05-29 11:21 MST
- Trips tracked total: 697
- New trips this scan: 5
