...
4. **Log-staleness trap (learned 2026-07-31 19:10-19:47 PT).** The upstream log writes via `with open(UPSTREAM_LOG, "a")` per completion — it opens/closes the file for EACH write. So there is no persistent FD, and `lsof` / `ls -l /proc/PID/fd` showing nothing is NORMAL, not a log outage. A "stale" log with zero recent entries means the adapter has had zero completed turns in that period — check the adapter's own `_log()` output (`/tmp/frankenstein_tools_adapter.log`) to confirm whether the adapter is alive but all turns are failing to complete (STREAM_PROXY failed, QOS_PREEMPT timeout, DECODE_STALL). **Never claim the log is "dead/broken" without first checking whether the adapter IS completing turns.** The test: `grep -c 'STREAM_PROXY ok' /tmp/frankenstein_tools_adapter.log` vs `stat` of the upstream log — if both show zero recent activity, the log is fine, the adapter is just not completing anything.
