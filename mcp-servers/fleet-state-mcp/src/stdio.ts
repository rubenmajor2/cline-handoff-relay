...
function readSnapshot(): CacheShape {
  try {
...
function readCached(action: CachedAction): unknown {
  ...
  if (!entry || entry.fetched_at === 0 || entry.data === undefined) {
    return {
      ...
      action,
      note:
        "Fleet snapshot not yet populated (the background refresher has not landed a snapshot since boot). " +
  ...
  const meta = {
    _cache: {
      ...
      note:
        "Served over STDIO from the local fleet snapshot (~/.fleet-state-mcp/snapshot.json, refreshed ~60s " +
...
async function remoteCall(
  ...
  timeoutMs: number = SELF_REFRESH_TIMEOUT_MS,
...
function persist(snap: CacheShape): void {
  try {
    ...
    console.error(`[fleet-state-stdio] persist failed (non-fatal): ${(e as Error).message}`);
  }
}

async function selfHealRefresh(): Promise<void> {
  ...
  try {
    ...
    console.error(`[fleet-state-stdio] selfHealRefresh failed (non-fatal): ${(e as Error).message}`);
  } finally {
    refreshing = false;
  }
}

function maybeSelfHeal(): void {
...
const TOOLS = [
  ...
  },
  {
    name: "fleet_now",
    description:
      "Live aggregate snapshot of EMSU fleet: host heartbeat ages, llm_call_log spend by model (last 24h), llm spend by surface (last 1h), recent fleet/runpod events, active RunPod pods, recent Fleet Agent decisions. Served over STDIO from the local snapshot with an _cache.age_seconds staleness marker. Use to answer 'what is the fleet doing right now'.",
    inputSchema: { type: "object", properties: {}, required: [] },
  },
  {
  ...
  },
  {
    ...
    description:
      "Queryable EMSU LLM routing map — idea #10160. Returns per-surface call stats (DEDUPED by request_id — eliminates 71-190x raw_rows inflation from streaming chunks), transport type, forced_claude flag, local_eligible flag, and corrected session facts. Served over STDIO from the local snapshot. " +
...
const server = new Server(
  { name: "fleet-state-mcp", version: "0.5.0" },
  { capabilities: { tools: {} } }
);
...
process.on("uncaughtException", (e: unknown) => {
  console.error(`[fleet-state-stdio] uncaughtException (swallowed): ${(e as Error)?.message || e}`);
});
