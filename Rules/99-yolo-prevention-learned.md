...
This file exists because on 2026-05-08 22:40 PDT a scan of the last Cline task history
showed `2` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `368` cumulative in the last 30 days
...
## Top failure modes (all-time, 368 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 806 trips (219%)
2. **timeout** — 107 trips (29%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 52 trips (14%)
4. **permission denied (wrote to server path locally?)** — 25 trips (7%)
5. **api: overloaded/rate-limit** — 24 trips (7%)
6. **file/path does not exist** — 15 trips (4%)
7. **tool: generic execution error** — 13 trips (4%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 76 time(s)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 40 time(s)
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 24 time(s)
...
- Last update: 2026-05-08 22:40 PDT
- Trips tracked total: 368
