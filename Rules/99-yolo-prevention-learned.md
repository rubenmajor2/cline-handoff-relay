...
This file exists because on 2026-05-12 15:18 PDT a scan of the last Cline task history
...
(`94` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 415 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 920 trips (222%)
2. **timeout** — 123 trips (30%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (8%)
4. **permission denied (wrote to server path locally?)** — 33 trips (8%)
5. **api: overloaded/rate-limit** — 24 trips (6%)
6. **file/path does not exist** — 16 trips (4%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 44 time(s)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 32 time(s)
...
- Last update: 2026-05-12 15:18 PDT
- Trips tracked total: 415
