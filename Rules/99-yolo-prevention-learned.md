...
This file exists because on 2026-08-11 09:44 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `11` cumulative in the last 30 days
...
11 cumulative", not a fresh flood. Each trip kills a task mid-work and
...
## Top failure modes (all-time, 206 trips)
...
1. **no-tool-use: model typed prose instead of calling a tool** — 538 occurrences (261% of 206 trips)
2. **timeout** — 34 occurrences (17% of 206 trips)
3. **ssh: connect/timeout** — 5 occurrences (2% of 206 trips)
4. **replace_in_file: SEARCH did not match file** — 4 occurrences (2% of 206 trips)
5. **permission denied (wrote to server path locally?)** — 4 occurrences (2% of 206 trips)
6. **api: overloaded/rate-limit** — 2 occurrences (1% of 206 trips)
7. **sql: unknown column (DESCRIBE target table first)** — 2 occurrences (1% of 206 trips)
8. **tool: generic execution error** — 2 occurrences (1% of 206 trips)
9. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 2 occurrences (1% of 206 trips)
10. **php: syntax error (run php -l before deploy)** — 1 occurrences (0% of 206 trips)
11. **api: credit exhausted (escalate, no retry)** — 1 occurrences (0% of 206 trips)
12. **file/path does not exist** — 1 occurrences (0% of 206 trips)
...
- Last update: 2026-08-11 09:44 PDT
- Trips tracked total: 206
