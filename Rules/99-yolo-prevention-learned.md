...
This file exists because on 2026-05-19 06:58 PDT a scan of the last Cline task history
...
(`54` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 464 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1030 trips (222%)
2. **timeout** — 129 trips (28%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 123 time(s)
...
- Last update: 2026-05-19 06:58 PDT
- Trips tracked total: 464
