...
This file exists because on 2026-05-21 20:13 PDT a scan of the last Cline task history
showed `3` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `503` cumulative in the last 30 days
...
## Top failure modes (all-time, 503 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1115 trips (222%)
2. **timeout** — 137 trips (27%)
3. **permission denied (wrote to server path locally?)** — 41 trips (8%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (7%)
5. **api: overloaded/rate-limit** — 27 trips (5%)
6. **file/path does not exist** — 18 trips (4%)
7. **tool: generic execution error** — 16 trips (3%)
8. **mysql query failed** — 13 trips (3%)
9. **ssh: connect/timeout** — 13 trips (3%)
10. **sql: unknown column (DESCRIBE target table first)** — 6 trips (1%)
11. **shell: command not found** — 4 trips (1%)
12. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
13. **php: syntax error (run php -l before deploy)** — 3 trips (1%)
14. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 3 trips (1%)
15. **safe-deploy: sha drift, re-read file before retry** — 2 trips (0%)
16. **browser_action: failed** — 2 trips (0%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 131 time(s)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 67 time(s)
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 40 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 26 time(s)
- `file/path does not exist > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 18 time(s)
- `tool: generic execution error > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 16 time(s)
...
- Last update: 2026-05-21 20:13 PDT
- Trips tracked total: 503
