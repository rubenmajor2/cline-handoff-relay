...
This file exists because on 2026-05-04 18:19 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `288` cumulative in the last 30 days
...
## Top failure modes (all-time, 288 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 613 trips (213%)
2. **timeout** — 116 trips (40%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 110 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 43 time(s)
...
- Last update: 2026-05-04 18:19 PDT
- Trips tracked total: 288
