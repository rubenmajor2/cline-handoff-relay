...
This file exists because on 2026-05-10 00:07 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `386` cumulative in the last 30 days
(`116` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 386 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 850 trips (220%)
2. **timeout** — 118 trips (31%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (9%)
4. **permission denied (wrote to server path locally?)** — 29 trips (8%)
...
- Last update: 2026-05-10 00:07 PDT
- Trips tracked total: 386
