...
This file exists because on 2026-06-01 20:36 PDT a scan of the last Cline task history
showed `4` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `87` cumulative in the last 30 days
...
## Top failure modes (all-time, 87 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 221 trips (254%)
2. **timeout** — 13 trips (15%)
3. **ssh: connect/timeout** — 4 trips (5%)
4. **replace_in_file: SEARCH did not match file** — 4 trips (5%)
5. **api: overloaded/rate-limit** — 2 trips (2%)
6. **sql: unknown column (DESCRIBE target table first)** — 2 trips (2%)
7. **tool: generic execution error** — 2 trips (2%)
8. **php: syntax error (run php -l before deploy)** — 1 trips (1%)
9. **api: credit exhausted (escalate, no retry)** — 1 trips (1%)
10. **permission denied (wrote to server path locally?)** — 1 trips (1%)
11. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 1 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 11 time(s)
...
- `php: syntax error (run php -l before deploy) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 1 time(s)
- `api: credit exhausted (escalate, no retry) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 1 time(s)
...
- **permission denied (wrote to server path locally?)** (1 trips) — see `clinerules_search("permission denied (wrote to server path locally?)")`
- **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** (1 trips) — see `clinerules_search("safe-deploy")`
...
- Last update: 2026-06-01 20:36 PDT
- Trips tracked total: 87
