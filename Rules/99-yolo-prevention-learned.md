...
This file exists because on 2026-05-03 23:30 MST a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `282` cumulative in the last 30 days
(`62` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 282 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 599 trips (212%)
...
6. **tool: generic execution error** — 14 trips (5%)
7. **ssh: connect/timeout** — 14 trips (5%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 41 time(s)
...
- Last update: 2026-05-03 23:30 MST
- Trips tracked total: 282
