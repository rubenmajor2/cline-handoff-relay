...
This file exists because on 2026-05-04 19:19 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `290` cumulative in the last 30 days
...
## Top failure modes (all-time, 290 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 617 trips (213%)
2. **timeout** — 117 trips (40%)
3. **api: overloaded/rate-limit** — 21 trips (7%)
4. **permission denied (wrote to server path locally?)** — 20 trips (7%)
5. **file/path does not exist** — 16 trips (6%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 111 time(s)
...
- Last update: 2026-05-04 19:19 PDT
- Trips tracked total: 290
