...
This file exists because on 2026-05-09 22:21 PDT a scan of the last Cline task history
...
## Top failure modes (all-time, 385 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 848 trips (220%)
2. **timeout** — 118 trips (31%)
3. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (9%)
4. **permission denied (wrote to server path locally?)** — 28 trips (7%)
...
11. **sql: unknown column (DESCRIBE target table first)** — 1 trips (0%)
12. **shell: command not found** — 1 trips (0%)
13. **ssh: port 2222 timeout** — 1 trips (0%)
14. **tool: result missing** — 1 trips (0%)
15. **php: syntax error (run php -l before deploy)** — 1 trips (0%)
16. **safe-deploy: invalid flag (use --target/--content/--expected-sha256)** — 1 trips (0%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 84 time(s)
...
- `permission denied (wrote to server path locally?) > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 27 time(s)
...
- Last update: 2026-05-09 22:21 PDT
- Trips tracked total: 385
