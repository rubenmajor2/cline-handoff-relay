...
This file exists because on 2026-06-09 23:37 PDT a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `176` cumulative in the last 30 days
(`37` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 176 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 461 trips (262%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 117 time(s)
...
- Last update: 2026-06-09 23:37 PDT
- Trips tracked total: 176
