...
This file exists because on 2026-05-05 03:30 MST a scan of the last Cline task history
showed `0` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `300` cumulative in the last 30 days
...
## Top failure modes (all-time, 300 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 646 trips (215%)
2. **timeout** — 111 trips (37%)
3. **api: overloaded/rate-limit** — 21 trips (7%)
4. **permission denied (wrote to server path locally?)** — 21 trips (7%)
5. **file/path does not exist** — 17 trips (6%)
6. **tool: generic execution error** — 15 trips (5%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 52 time(s)
...
- Last update: 2026-05-05 03:30 MST
- Trips tracked total: 300
