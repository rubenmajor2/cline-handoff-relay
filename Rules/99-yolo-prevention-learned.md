...
This file exists because on 2026-05-25 15:43 PDT a scan of the last Cline task history
...
(`133` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 593 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1321 trips (223%)
2. **timeout** — 154 trips (26%)
3. **permission denied (wrote to server path locally?)** — 45 trips (8%)
4. **fpm-reload: sudoers blocks systemctl, use kill -USR2 wrapper** — 35 trips (6%)
5. **api: overloaded/rate-limit** — 29 trips (5%)
6. **file/path does not exist** — 22 trips (4%)
7. **tool: generic execution error** — 20 trips (3%)
8. **mysql query failed** — 15 trips (3%)
9. **ssh: connect/timeout** — 15 trips (3%)
10. **sql: unknown column (DESCRIBE target table first)** — 11 trips (2%)
11. **php: syntax error (run php -l before deploy)** — 7 trips (1%)
12. **browser_action: failed** — 7 trips (1%)
...
16. **safe-deploy: sha drift, re-read file before retry** — 3 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 147 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 141 time(s)
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > (none)` — 79 time(s)
...
- `ssh: connect/timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 15 time(s)
...
- **file/path does not exist** (22 trips) — see `clinerules_search("file/path does not exist")`
...
- **ssh: connect/timeout** (15 trips) — see `clinerules_search("ssh")`
- **sql: unknown column (DESCRIBE target table first)** (11 trips) — see `clinerules_search("sql")`
- **php: syntax error (run php -l before deploy)** (7 trips) — see `clinerules_search("php")`
- **browser_action: failed** (7 trips) — see `clinerules_search("browser_action")`
...
- Last update: 2026-05-25 15:43 PDT
- Trips tracked total: 593
