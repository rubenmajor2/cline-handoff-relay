...
## Mechanical amendment trail (moved from Rules/297 on 2026-08-19, trim-then-archive)

These 31 amendment blocks were appended by clinerules_amend_rule. Moved here to
restore G7/G8 compliance (the floor file had grown to 44,901 bytes; cap 12,288).
The distilled lessons are folded into the parent rule's gate text; this is the
verbatim proof trail.
...
## Mechanical amendments (moved from Rules/297-population-anomaly-classify-before-alarming.md on 2026-08-19 ~18:16 PT, concurrent-window batch)

## Amendment trail — moved (2026-08-19)

31 mechanical amendments (2026-08-17 through 2026-08-19) appended by
clinerules_amend_rule grew this file to 44,901 bytes (3.7x the G7 cap), failing
the G8 floor gate that protects every window's system prompt. The verbatim trail
now lives in `Rules-archive/297-case-law.md` (section "Mechanical amendment
trail"). The distilled lessons are folded into the gates above:

- **Read the source that produced the symptom** — log correlation is not causal
  classification; reproduce the exact error before naming a code path. A code
  default is a hypothesis about config; resolve the EFFECTIVE runtime value
  (systemd drop-ins, actual config layering) before attributing behavior to a
  configurable constant.
- **Read the WHOLE tool output** — read detail rows, not just headers; read the
  designated error/reason column before attributing cause from an adjacent output
  column; read terminal-outcome fields and the author before per-entity claims.
- **Standing lifecycle tools first** — run get_student_lifecycle_state /
  verify_payment_state / lookup_paperwork_state BEFORE hand-rolling targeted
  queries; they enumerate gates a hypothesis would never think to ask about.
- **Identify the writer before assigning cause** — a missing row is evidence
  something did not happen, never evidence of WHO failed; check whether a
  user-completed activity governs the state before calling it org fault. A status
  enum/boolean is NOT evidence the artifact it describes exists — query the
  holding table.
- **Read the document before characterizing it** — an email that says "see
  attached" is not the attachment; a cover email is not the complaint; a topic
  label or prior draft's framing is a hypothesis about what is charged.
- **Access = the access log, not provisioning** — an enrolment row or outbound
  send is what the PROGRAM did; query the server-side access log for what the
  USER experienced.
- **Repair scope = every surface** — enumerate every surface that can produce the
  same output before declaring a defect fixed; a coverage gate written after the
  repair is the proof of coverage.
- **Regulator filings** — answer the allegation actually made and nothing else;
  dissect element by element; never volunteer uncharged subjects "for
  completeness"; headings in neutral record vocabulary (now folded into rule 302).
- **Physical/hardware claims** — verify enumeration before claiming physical
  state from sysfs; name the containing register (SltSta vs SltCtl); kernel
  ordinals (bus numbers, ifindex) are not stable identifiers; read the bracketed
  selection in multi-option sysfs files; read back register writes before
  concluding from their effect.
- **vLLM capacity math** — compare 'KV cache size: N tokens' directly against the
  target context; never scale a pool-size GiB number by a context ratio; the
  concurrency multiplier line is ground truth.
- **Wrong-backend symptoms** — enumerate the FULL selection/fallback table first;
  when the model emits the wrong output SHAPE, read the steering text that
  defined the shape before blaming transport/capacity.
- **ID key spaces** — before declaring an ID mapping wrong, resolve WHICH
  system's key space the ids live in (Moodle assign ids vs portal form ids).
- **Scope-gate population checks** — enumerate the outcome space, state window +
  population, sanity-check against the user's estimate, and scan for corroborating
  inbound before escalating mass impact (steps 1-6 of the SCOPE GATE above).
