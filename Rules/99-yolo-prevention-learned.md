...
This file exists because on 2026-05-05 04:20 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `317` cumulative in the last 30 days
...
## Top failure modes (all-time, 317 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 684 trips (216%)
2. **timeout** — 127 trips (40%)
3. **api: overloaded/rate-limit** — 22 trips (7%)
4. **permission denied (wrote to server path locally?)** — 20 trips (6%)
5. **file/path does not exist** — 16 trips (5%)
6. **ssh: connect/timeout** — 16 trips (5%)
7. **tool: generic execution error** — 12 trips (4%)
8. **mysql query failed** — 8 trips (3%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 56 time(s)
...
- `api: overloaded/rate-limit > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 21 time(s)
...
- Last update: 2026-05-05 04:20 PDT
- Trips tracked total: 317
