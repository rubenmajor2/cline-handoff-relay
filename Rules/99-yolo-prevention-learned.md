...
This file exists because on 2026-06-05 12:07 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `159` cumulative in the last 30 days
...
## Top failure modes (all-time, 159 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 413 trips (260%)
2. **timeout** — 21 trips (13%)
...
- `no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 101 time(s)
...
- Last update: 2026-06-05 12:07 PDT
- Trips tracked total: 159
