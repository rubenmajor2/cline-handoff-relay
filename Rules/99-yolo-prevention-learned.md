...
This file exists because on 2026-05-25 12:42 PDT a scan of the last Cline task history
showed `1` new "[YOLO MODE] Task failed: Too many consecutive mistakes (3)"
trips this scan, against `364` cumulative in the last 30 days
(`120` in the last 7 days). The cumulative number doesn't grow unless
...
## Top failure modes (all-time, 579 trips)

1. **no-tool-use: model typed prose instead of calling a tool** — 1289 trips (223%)
2. **timeout** — 148 trips (26%)
...
13. **plan: no terminal action (must include safe_deploy/sql_execute/write/send)** — 4 trips (1%)
14. **php: syntax error (run php -l before deploy)** — 4 trips (1%)
...
- `timeout > no-tool-use: model typed prose instead of calling a tool > no-tool-use: model typed prose instead of calling a tool` — 141 time(s)
...
- **php: syntax error (run php -l before deploy)** (4 trips) — see `clinerules_search("php")`
...
- Last update: 2026-05-25 12:42 PDT
- Trips tracked total: 579
